...
final class Renderer: NSObject, MTKViewDelegate {
    ...
    init?(mtkView: MTKView) {
        ...
        // Prefer the precompiled default.metallib; fall back to compiling
        // Shaders.metal at runtime (works with Command Line Tools only,
        // no full Xcode required).
        let library: MTLLibrary
        if let lib = device.makeDefaultLibrary() {
            library = lib
        } else if let url = Bundle.main.url(forResource: "Shaders", withExtension: "metal"),
        ...
        guard let vfn = library.makeFunction(name: "boid_vertex"),
